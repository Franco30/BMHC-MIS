...
if(isset($_POST["From"], $_POST["to"]))
{
	...
	$result = '';
	$query = "SELECT * FROM `patient_child` WHERE `date` BETWEEN '".$_POST["From"]."' AND '".$_POST["to"]."' ORDER BY `date` DESC";
	...
	$q2 = "SELECT COUNT(*) AS `total` FROM `patient_child` WHERE `date` BETWEEN '".$_POST["From"]."' AND '".$_POST["to"]."'";
	...
	if(mysqli_num_rows($sql) > 0)
	{
		while($row = mysqli_fetch_array($sql))
		{
			$result .='
			<tr>
			<td><center><strong>'.$row["year"].'0'.$row["child_id"].'</center></strong></td>
