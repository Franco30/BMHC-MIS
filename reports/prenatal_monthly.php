<div class="monthly barmonth splinemonth4 piemonth dougnutmonth reporttype" style="display: none;">
    ...
    <table id="prenatalmonthlytable" class="table table-bordered table-condensed nowrap" width="100%">
        ...
        <tbody>
            ...
            $query1 = $conn->query("SELECT count(*) as permonth FROM `prenatal` where `year` = '$year'") or die(mysqli_error());
            $fetch1 = $query1->fetch_array();
            $query = $conn->query("SELECT month, count(*) as count FROM `prenatal` where `year` = '$year' group by month order by count DESC") or die(mysqli_error());
