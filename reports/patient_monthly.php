<div class="1monthly reporttype" style="display: none;">
    ...
    <table id="patientmonthlytable" class="table table-bordered table-condensed nowrap" width="100%">
        <thead>
            <tr class="warning">
                <th>Month</th>
                <th>Total Count</th>
        ...
        <tbody>
            ...
            $query1 = $conn->query("SELECT count(*) as permonth FROM `patient` where `year` = '$year'") or die(mysqli_error());
            $fetch1 = $query1->fetch_array();
            $query = $conn->query("SELECT month, count(*) as count FROM `patient` where `year` = '$year' group by month order by count DESC") or die(mysqli_error());
