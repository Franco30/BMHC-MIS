<div class="1monthly reporttype" style="display: none;">
    ...
    <table id="patient_childmonthlytable" class="table table-bordered table-condensed nowrap" width="100%">
        ...
        <tbody>
            ...
            $query1 = $conn->query("SELECT count(*) as permonth FROM `patient_child` where `year` = '$year'") or die(mysqli_error());
            $fetch1 = $query1->fetch_array();
            $query = $conn->query("SELECT month, count(*) as count FROM `patient_child` where `year` = '$year' group by month order by count DESC") or die(mysqli_error());
...
?>
<hr>
<h4>Total Child Patients for the Year <?php echo $year. ' : <strong> ' .$fetch['total'].' Child Patients</strong>'?></h4> <hr>
