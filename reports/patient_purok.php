<div class="1colpatientpurok reporttype" style="display: none;">
    ...
    <table id="patientmonthlytable" class="table table-bordered table-condensed nowrap" width="100%">
        ...
        <tbody>
            ...
            $query1 = $conn->query("SELECT count(*) as bypurok FROM `patient` where `year` = '$year'") or die(mysqli_error());
            $fetch1 = $query1->fetch_array();
            $query = $conn->query("SELECT purok, count(*) as count FROM `patient` where `year` = '$year' group by purok order by count DESC") or die(mysqli_error());
